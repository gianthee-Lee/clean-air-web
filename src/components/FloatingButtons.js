...
export default function FloatingButtons({ phone, kakaoUrl }) {
  const [show, setShow] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      // 100px 이상 스크롤하면 버튼 표시
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3">
      ...
      {phone && (
        <a
          href={`tel:${phone}`}
          className="w-14 h-14 bg-white border border-[var(--color-border)] rounded-full shadow-lg flex items-center justify-center text-[var(--color-primary)] hover:bg-[var(--color-primary-light)] transition-all hover:-translate-y-1 group"
          title="전화 문의"
        >
      ...
      {kakaoUrl && (
        <a
          ...
          className="w-14 h-14 bg-[#FEE500] rounded-full shadow-lg flex items-center justify-center text-[#191919] hover:bg-[#FADA0A] transition-all hover:-translate-y-1 group"
          title="카카오톡 문의"
        >
          <svg
            stroke="currentColor"
